/**
 * Terminal — fullscreen ttyd iframe with a clean iOS key bar
 *
 ...
 *
 * Keys are sent via POST /api/terminal/keys → tmux send-keys on the server,
 * completely bypassing iOS's isTrusted / iframe keyboard restrictions.
 */
import { motion, AnimatePresence } from 'framer-motion';
import { useState, useEffect, useRef, useCallback } from 'react';
import { RefreshCw, Settings2, Terminal as TermIcon, ArrowRight,
         WifiOff, Loader, Keyboard, ChevronDown } from 'lucide-react';
...
function KeyBar({ base, onClose }: KeyBarProps) {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-0 left-0 right-0 z-30 bg-[#141418]/95 backdrop-blur-md border-t border-white/8 pb-safe"
    >
      {/* drag-to-close pill */}
      <button
        type="button"
      ...
      <div className="flex items-center gap-1 px-2 pb-2 overflow-x-auto scrollbar-none w-max min-w-full">
        {ctrlActive ? (
          ...
          // Nano / readline shortcuts (most common in daily use)
          <>
