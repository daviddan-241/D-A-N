...
extract_port() {
  ...
  local p
  p=$(grep -oE 'bore\.pub:[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
while true; do
  ...
  [[ -n "${p}" ]] && echo "${p}" && return
  p=$(grep -oE 'remote_port=[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
while true; do
  ...
  [[ -n "${p}" ]] && echo "${p}" && return
  p=$(grep -oE 'Listening at [^:]+:[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
while true; do
  ...
  [[ -n "${p}" ]] && echo "${p}" && return
  p=$(grep -oE 'port[ =:][0-9]{4,5}' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
