...
if [[ "${BORE_ENABLE:-no}" == "yes" ]] && command -v bore &>/dev/null; then
  ...
  # "see log" forever.
  extract_bore_port() {
    # Try multiple patterns to handle different bore versions / output formats
    grep -oE 'bore\.pub:[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
# Ensure /var/run/sshd exists — on some Render images /var/run is a tmpfs
# that doesn't preserve directories from the Dockerfile layer.
...
exec node --enable-source-maps ./dist/index.mjs 2>&1 | tee -a "${LOG_DIR}/app.log"
 && return
    grep -oE 'remote_port=[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
exec node --enable-source-maps ./dist/index.mjs 2>&1 | tee -a "${LOG_DIR}/app.log"
 && return
    grep -oE 'Listening at [^:]+:[0-9]+' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
...
exec node --enable-source-maps ./dist/index.mjs 2>&1 | tee -a "${LOG_DIR}/app.log"
 && return
    grep -oE 'port[ =:][0-9]{4,5}' "${LOG_DIR}/bore.log" 2>/dev/null | tail -1 | grep -oE '[0-9]+
